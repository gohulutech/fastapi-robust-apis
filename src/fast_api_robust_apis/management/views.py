...
from fast_api_robust_apis.db.session import SessionLocal
from fast_api_robust_apis.crud import (
    product_repository,
    location_repository,
...
def view_all_locations():
    ...
    try:
        ...
        for loc, stock in locations_with_stock:
            ...
            address = (
                loc.address[:30] + "..."
                if loc.address and len(loc.address) > 30
                else loc.address
            )

            table_data.append(
                [loc.id, loc.name, address, loc.capacity, stock, f"{utilization:.1f}%"]
            )
...
def view_inventory_by_product():
    ...
    try:
        ...
        for inv, loc in inventory:
            ...
            table_data.append([loc.name, inv.quantity, inv.reorder_point, status])
...
def view_inventory_by_location():
    ...
    try:
        ...
        for inv, prod in inventory:
            status = "LOW STOCK" if inv.quantity < inv.reorder_point else "OK"
            table_data.append(
                [prod.name, prod.sku, inv.quantity, inv.reorder_point, status]
            )
            ...
            try:
                ...
                if input_value.lower().startswith("set "):
                    ...
                    try:
                        ...
                        if (
                            quantity_change < 0
                            and abs(quantity_change) > current_quantity
                        ):
                            print(
                                f"Cannot remove {abs(quantity_change)} units. Only {current_quantity} available."
                            )
                            continue
                        ...
                        if result:
                            print(
                                f"Stock updated successfully. New quantity: {result.quantity}"
                            )
                        else:
                        ...
                        print(
                            "Please enter a valid number or 'set' followed by a number."
                        )
            except Exception as e:
                print(f"Error: {str(e)}")
